...
def callback(data):
    ...
    for detection in data.detections.detections:
        if detection.results[0].id == 0:
            ...
            print(Fore.YELLOW + str(detection.bbox.center.x) + " ; " + str(detection.bbox.center.y)
                  + "  ;  " + str(detection.results[0].score))
        if detection.results[0].id == 63:
            print("Laptop detected: ", end='')
            print(Fore.GREEN + str(detection.bbox.center.x) + " ; " + str(detection.bbox.center.y)
                  + "  ;  " + str(detection.results[0].score))
...
def main():
    ...
    # Setup Ros
    rospy.Subscriber(topic, YoloResult, callback)
